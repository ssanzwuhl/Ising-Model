...
class Variable:
    def __init__(self):
        self.values = [] #The list upon which the measurements of the variable are stored
        self.errors = [] #The list of the errors of each measurement
    ...
    def measurement(self):
        ...
        while True:
            ...
            self.values.append(float(measure))
...
class new_experiment:
    ...
    def run_experiment(self):  #When an experiment is ran, we only measure one
        ...
        self.variables[self.variable_names.index(dependent_var)].values = np.linspace(min_value, max_value, num=N)
